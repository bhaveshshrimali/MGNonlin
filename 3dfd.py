...
#    print(gradU.reshape(geom.nDim,geom.nDim,-1))
    ...
    """
    F=gradU+np.array([1,0,0,
                      0.,1,0,
                      0,0,1]).reshape(-1,1,1).repeat(len(Xi),axis=-1)      #convert to 3x3xNgP and the take det(F) and inv(F)
...
dof[fdof]=0.
lineardof=dof.copy()
DfGrn=([])                   # Deformation Gradient
Strs=([])                    # First-PK Stress
LagStrain=([])               # Lagrangian Strain
epss=([])
intpt=([])
# Linear FEM (as a special case of Nonlinear)
intpt1=assembly(dof.reshape(-1,3).T.flatten())['IntP'][0].T    #global gauss-point
Ks=assembly(lineardof.reshape(-1,3).T.flatten())['Jac']  #linear stiffness
lineardof[(prescribed_dofs[:,0]).astype(int)]=prescribed_dofs[:,1]
Fs=assembly(lineardof.reshape(-1,3).T.flatten())['rhs']
lineardof[fdof]=la.solve(Ks[np.ix_(fdof,fdof)],-Ks[np.ix_(fdof,nfdof)]@lineardof[nfdof])
linearStress=assembly(lineardof.reshape(-1,3).T.flatten())['S'] 
linearStrain=assembly(lineardof.reshape(-1,3).T.flatten())['SmlStrain']
##
dofstore=np.zeros(dof.shape);
#Strn=np.zeros((3,3,8))
#Strs=np.zeros((3,3,8))
#Fs=assembly(dof.reshape(-1,3).T.flatten())['rhs']
#
for i in range(geom.nSteps):
    ...
    epss.append(assembly(dof)['SmlStrain'])
...
plt.figure()
plt.plot(np.linspace(0,1,Strs[:,-1,-1,-1].size),Strs[:,-1,-1,-1],label=r'$\sigma_{33}$')
plt.xlabel(r'$t$ (Time)',fontsize=20)
plt.ylabel(r'$\sigma_{33}$ (Axial Stress)',fontsize=20)
