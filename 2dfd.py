...
class DWDIi():               
    def __init__(self,ndim):
        ...
        W = 1/2.*geom.mu*(I1-2.)-geom.mu*(J-1.)+geom.lam/2.*(J-1.)**2               #change W here to include the modified Neo-Hookean
        dWdI1=W.diff(I1,1)
        dWdJ=W.diff(J,1)
#        print(dWdJ)
        d2WdI12=W.diff(I1,2);
...
def assembly(disp):
    globK=sp.lil_matrix(1.e-15*np.eye(disp.size))
...
dof=-20*np.ones(meshxy.size) #initializing dofs (displacement of nodes)
intpt=([])
...
dofs_top_yval = 0.55*geom.ylength*np.ones(dofs_top_y.size)
...
fdof=dof==-20.                       #free dofs flags: further initialization to zeros needed only for the first step 
...
#        ml = pmg.smoothed_aggregation_solver(Ks1[np.ix_(fdof,fdof)],max_levels=20)
        del_dof = ml.solve(-Fs1[fdof],-Fs1[fdof],tol=1.e-9,accel='cg',residuals=res)        #np.random.random(Ks1[np.ix_(fdof,fdof)].shape[0])
...
#plt.tricontourf(meshxy[:,0],meshxy[:,1],dof[np.arange(1,dof.size,2)],cmap=plt.cm.get_cmap('RdBu'))
#plt.colorbar()