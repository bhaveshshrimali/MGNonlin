...
#        print('Elast:\n',S[:,:,plsTidx])
    Splast = S.copy() - 2*geom.mu*(eplast+dgam*nNp1).reshape(9,1,geom.NGp)
    S[:,:,plsTidx] = Splast[:,:,plsTidx]
#        print('Plast\n:',S[:,:,plsTidx])
...
dof[fdof]=0.
lineardof=dof.copy()
DfGrn=np.zeros((len(conVxyz),geom.T.size,geom.nDim,geom.nDim,geom.NGp))                  # Deformation Gradient
Strs=np.zeros((len(conVxyz),geom.T.size,geom.nDim,geom.nDim,geom.NGp))                    # First-PK Stress
LagStrain=np.zeros((len(conVxyz),geom.T.size,geom.nDim,geom.nDim,geom.NGp))               # Lagrangian Strain
epss=np.zeros((len(conVxyz),geom.T.size,geom.nDim,geom.nDim,geom.NGp)) 
intpt=([])
...
Eplastic=np.zeros((len(conVxyz),geom.nDim,geom.nDim,geom.NGp,geom.nSteps))                     #initialize the plastic strain
intpt1=assembly(dof,qGlobal[:,:,:,:,0],alphGlobal[:,:,0],Eplastic[:,:,:,:,0])['IntP'][0].T    #global gauss-point locations (?)
dofstore=np.zeros(dof.shape);
Ks3=assembly(dof,qGlobal[:,:,:,:,0],alphGlobal[:,:,0],Eplastic[:,:,:,:,0])['Jac']  #linear stiffness for a check 
#
for i in range(geom.nSteps):
    dof[(prescribed_dofs[:,0]).astype(int)]=(i+1)/(geom.nSteps)*prescribed_dofs[:,1]
    Ks1=assembly(dof,qGlobal[:,:,:,:,i],alphGlobal[:,:,i],Eplastic[:,:,:,:,i])['Jac']
    Fs1=assembly(dof,qGlobal[:,:,:,:,i],alphGlobal[:,:,i],Eplastic[:,:,:,:,i])['rhs'] 
    normres=la.norm(Fs1[fdof],2)
    iterNR=0;
    qtemp=qGlobal[:,:,:,:,i].copy();
    alphtemp=alphGlobal[:,:,i].copy()
    Eplastictemp=Eplastic[:,:,:,:,i].copy()
    while normres >= geom.tolNR and iterNR<=geom.maxiter:        
        ...
        Ks1=assembly(dof,qtemp,alphtemp,Eplastictemp)['Jac']
        Fs1=assembly(dof,qtemp,alphtemp,Eplastictemp)['rhs']
#        qtemp=assembly(dof,qtemp,alphtemp,Eplastictemp)['qGlob']
#        alphtemp=assembly(dof,qtemp,alphtemp,Eplastictemp)['Alpha']
        Eplastictemp=assembly(dof,qtemp,alphtemp,Eplastictemp)['EP']
        normres=la.norm(Fs1[fdof],2)
        print('Step: {}'.format(i+1))
        iterNR += 1
    dofstore=np.vstack((dofstore,dof))
    DfGrn[i+1]=assembly(dof,qGlobal[:,:,:,:,i],alphGlobal[:,:,i],Eplastic)['F']
    Strs[i+1]=assembly(dof,qGlobal[:,:,:,:,i-1],alphGlobal[:,:,i-1],Eplastic)['S']
    LagStrain[i]=assembly(dof,qGlobal[:,:,:,:,i-1],alphGlobal[:,:,i-1],Eplastic)['E']
    epss[i]=assembly(dof,qGlobal[:,:,:,:,i-1],alphGlobal[:,:,i-1],Eplastic)['SmlStrain']
    qGlobal[:,:,:,:,i]=assembly(dof,qGlobal[:,:,:,:,i-1],alphGlobal[:,:,i-1],Eplastic)['qGlob']
    alphGlobal[:,:,i]=assembly(dof,qGlobal[:,:,:,:,i-1],alphGlobal[:,:,i-1],Eplastic)['Alpha']
    Eplastic=assembly(dof,qGlobal[:,:,:,:,i-1],alphGlobal[:,:,i-1],Eplastic)['EP']
#
##intpt=
