...
class geometry() : #1D geometry
    def __init__(self,Eltype,Tf,E,nu):
        ...
        if Eltype[0]=='L':
            ...
            self.T=np.linspace(0.,Tf,self.nSteps)
...
#    S=WpI1*2*F+(WpJ*detF).reshape(1,1,-1)*Finv[np.array([0,3,6,1,4,7,2,5,8],int)]       #notice the swap of axes for transpose (of the inverse)
    ...
    S=2*geom.mu*epSS+geom.lam*np.einsum('iik->k',epSS)*np.eye(geom.nDim).reshape(3,3,-1).repeat(geom.NGp,axis=-1)
    S=S.reshape(-1,1,geom.NGp)
#    devEps=epSS-1./3*np.einsum('iik->k',epSS)*np.eye(geom.nDim).reshape(3,3,-1).repeat(geom.NGp,axis=-1)
#    strial=2*geom.mu*(devEps-eplast)
    trS=np.einsum('iik->k',S.reshape(geom.nDim,geom.nDim,-1))
#    print(trS)
    sf=S.reshape(geom.nDim,geom.nDim,-1)-1./3*trS*np.eye(geom.nDim).reshape(geom.nDim,geom.nDim,-1).repeat(geom.NGp,axis=-1)  #stress deviator at GP
    sdev=sf-2*geom.mu*eplast     #trial deviator
    sEff=sdev-qe
    ftrial = la.norm(sEff,2,axis=(0,1)) - (2./3)**0.5*(geom.Ko+geom.Kp*alphe)   # Yield-surface boundary (NGp)
#    print(qe)
    plsTidx = ftrial > 0.    # boolean array to keep track of plasticity(True-Plastic, False-Elastic!)
#    print(plsTidx)
    dgam=np.zeros(plsTidx.size)  # initialize dgam to all zeros assuming elastic response
...
#        print('here')
        ...
        dgam[plsTidx]=3*ftrial[plsTidx]/(2*(geom.Hp+geom.Kp+3*geom.mu))    #evolve dgam only at the plastified gauss points 
        nNp1=sEff/la.norm(sEff,2,axis=(0,1))    #n_{n+1}
        alphe += (2./3)**0.5*dgam  
        qe += 2./3*geom.Hp*dgam*nNp1            #update qe in place and store in n+1 
#        print((dgam*nNp1).shape)
#        S[:,:,plsTidx] = (geom.kap*np.einsum('iik->k',epSS)*np.eye(geom.nDim).reshape(3,3,-1).repeat(geom.NGp,
#         axis=-1)+strial-2*geom.mu*dgam*nNp1).reshape(-1,1,geom.NGp)[:,:,plsTidx]         #Original shape of stress (--check ?? )
#        print('original',S[:,:,plsTidx])
#        print('Elast:\n',S[:,:,plsTidx]) 
    S -= 2*geom.mu*(eplast+dgam*nNp1).reshape(-1,1,geom.NGp)
#    if istep >=65:
#    print(S[-1,-1,plsTidx])
    Splast=S.copy()
    eplast += dgam*nNp1
...
#        print('changed',S[:,:,plsTidx])
    fac=Wg*detJ
#    print(eplast[-1,-1,-1])
    res=np.einsum('lik,ljk->ij',Bmat,fac*Splast)                                       #double contraction along axis 1 and 2 (of B)
    """
...
#        print('thtab:\n',thtab)
        ...
        C[:,:,plsTidx]=Cep[:,:,plsTidx]            #only take values for those gauss points which are plastified
...
#    Strn=np.zeros((len(conVxyz),9,1,geom.NGp))
    for i in range(len(conVxyz)):
        elnodes=conVxyz[i]
#        print(elnodes)
        ep=Ep[i]
        qe=q[i]
#        print('globshao=',qe.shape)
        alphe=alph[i]
...
#        print(globK)
        ...
        strs=locmat(nodexy.T.flatten(),locdisp,qe,alphe,ep)['Stress']
        DG=locmat(nodexy.T.flatten(),locdisp,qe,alphe,ep)['DefGrad']
#        calculate strains and integration point coordinates
        Strn=(np.einsum('lik,ljk->ijk',DG.reshape(geom.nDim,geom.nDim,-1),DG.reshape(geom.nDim,geom.nDim,-1))-np.eye(geom.nDim).reshape(geom.nDim,geom.nDim,-1).repeat(8,axis=-1))/2
#        print('what:',strs[-1,-1,-1])
        epsStrn=(np.einsum('ijk->jik',DG.reshape(geom.nDim,geom.nDim,-1))+DG.reshape(geom.nDim,geom.nDim,-1)-2*np.eye(geom.nDim).reshape(geom.nDim,geom.nDim,-1).repeat(8,axis=-1))/2
        intpt.append(locmat(nodexy.T.flatten(),locdisp,qe,alphe,ep)['InptGlobal'])
#        Store the updated plastic strains, q, and alpha for all the elements
        q[i]=locmat(nodexy.T.flatten(),locdisp,qe,alphe,ep)['Q']
        Ep[i]=locmat(nodexy.T.flatten(),locdisp,qe,alphe,ep)['PlsStrn']
#        print('shp=',ep.shape)
        alph[i]=locmat(nodexy.T.flatten(),locdisp,qe,alphe,ep)['Alph']
#        print(epsStrn[:,:,-1])
    return {'Jac':globK,
            'rhs':globF,
...
dof[fdof]=0.
lineardof=dof.copy()
DfGrn=np.zeros((geom.T.size,geom.nDim,geom.nDim,geom.NGp))                  # Deformation Gradient
Strs=np.zeros((geom.T.size,geom.nDim,geom.nDim,geom.NGp))                   # First-PK Stress
LagStrain=np.zeros((geom.T.size,geom.nDim,geom.nDim,geom.NGp))              # Lagrangian Strain
epss=np.zeros((geom.T.size,geom.nDim,geom.nDim,geom.NGp)) 
intpt=([])
qGlobal=np.zeros((len(conVxyz),geom.nDim,geom.nDim,geom.NGp,geom.nSteps))    #check dimensions once again though
alphGlobal=np.zeros((len(conVxyz),geom.NGp,geom.nSteps))                     #just a scalar
Eplastic=np.zeros((len(conVxyz),geom.nDim,geom.nDim,geom.NGp))                     #initialize the plastic strain
intpt1=assembly(dof,qGlobal[:,:,:,:,0],alphGlobal[:,:,0],Eplastic)['IntP'][0].T    #global gauss-point locations (?)
dofstore=np.zeros(dof.shape);
Ks3=assembly(dof,qGlobal[:,:,:,:,0],alphGlobal[:,:,0],Eplastic)['Jac']  #linear stiffness
#
for istep in range(geom.nSteps):
    dof[(prescribed_dofs[:,0]).astype(int)]=(istep+1)/(geom.nSteps)*prescribed_dofs[:,1]
    Ks1=assembly(dof,qGlobal[:,:,:,:,istep-1],alphGlobal[:,:,istep-1],Eplastic)['Jac']
    Fs1=assembly(dof,qGlobal[:,:,:,:,istep-1],alphGlobal[:,:,istep-1],Eplastic)['rhs'] 
    normres=la.norm(Fs1[fdof],2)
    iterNR=0;
    qtemp=qGlobal[:,:,:,:,istep-1].copy();
    alphtemp=alphGlobal[:,:,istep-1].copy()
#    Eplastictemp=Eplastic.copy()
    while normres >= geom.tolNR and iterNR<=geom.maxiter:        
        ...
        Ks1=assembly(dof,qtemp,alphtemp,Eplastic)['Jac']
        Fs1=assembly(dof,qtemp,alphtemp,Eplastic)['rhs']
        qtemp=assembly(dof,qtemp,alphtemp,Eplastic)['qGlob']
        alphtemp=assembly(dof,qtemp,alphtemp,Eplastic)['Alpha']
        Eplastic=assembly(dof,qtemp,alphtemp,Eplastic)['EP']
        normres=la.norm(Fs1[fdof],2)
#        print('Iter: {}'.format(iterNR+1))
        iterNR += 1
    print('Step: {}'.format(istep+1))
#    if istep==66:
#        print(Strs[istep-1,-1,-1,-1])
##        break
    dofstore=np.vstack((dofstore,dof))
    DfGrn[istep]=assembly(dof,qGlobal[:,:,:,:,istep-1],alphGlobal[:,:,istep-1],Eplastic)['F']
    Strs[istep]=assembly(dof,qGlobal[:,:,:,:,istep-1],alphGlobal[:,:,istep-1],Eplastic)['S']
#    print(Strs[i,-1,-1,-1])
    LagStrain[istep]=assembly(dof,qGlobal[:,:,:,:,istep-1],alphGlobal[:,:,istep-1],Eplastic)['E']
    epss[istep]=assembly(dof,qGlobal[:,:,:,:,istep-1],alphGlobal[:,:,istep-1],Eplastic)['SmlStrain']
    qGlobal[:,:,:,:,istep]=assembly(dof,qGlobal[:,:,:,:,istep-1],alphGlobal[:,:,istep-1],Eplastic)['qGlob']
    alphGlobal[:,:,istep]=assembly(dof,qGlobal[:,:,:,:,istep-1],alphGlobal[:,:,istep-1],Eplastic)['Alpha']
    Eplastic=assembly(dof,qGlobal[:,:,:,:,istep-1],alphGlobal[:,:,istep-1],Eplastic)['EP']
#
##intpt=
DfGrn=np.array(DfGrn)
Strs=np.array(Strs)
LagStrain=np.array(LagStrain) 
epss=np.array(epss)
#intpt=np.array(intpt)
...
plt.plot(geom.T,Strs[:,0,0,-1],label=r'$\sigma_{11}$')
