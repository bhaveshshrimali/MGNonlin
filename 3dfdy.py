...
def locmat(nodes,de):                     #local stiffness (jacobian) and force (residual) over the reference element
    ...
    aaa.append(np.array(B.Ns(Xi,Eta,Rho)))
    gDshpL=np.array(B.dN(Xi,Eta,Rho)).reshape(geom.nDim,len(Xi),-1)                #local derivatives
...
intpt1=assembly(dof)['IntP'][0].T    #global gauss-point
Ks=assembly(lineardof)['Jac']  #linear stiffness
lineardof[(prescribed_dofs[:,0]).astype(int)]=prescribed_dofs[:,1]
Fs=assembly(lineardof)['rhs']
lineardof[fdof]=la.solve(Ks[np.ix_(fdof,fdof)],-Ks[np.ix_(fdof,nfdof)]@lineardof[nfdof])
linearStress=assembly(lineardof)['S'] 
linearStrain=assembly(lineardof)['SmlStrain']
##
